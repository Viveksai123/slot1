...
const Levelone = ({ username, rollnum, initialScore, timeLeft }) => {
  ...
  const handleCastSpell = () => {
    const dummyPassword = 'keylogger'; // Dummy password for testing
  ...
  const handleNextLevel = async () => {
    ...
    try {
      ...
      const fetchResponse = await fetch(`https://json-production-72ef.up.railway.app/records?rollnum=${rollnum}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (fetchResponse.ok) {
        ...
        if (existingDataArray && existingDataArray.length > 0) {
          ...
          if (currentLevel === existingData.level + 1) {
            ...
            const updateResponse = await fetch(`https://json-production-72ef.up.railway.app/records/${existingData.id}`, {
          ...
          <div className="popup-content">
            ...
            {isSpellValidated ? (
              ...
              <div className='column'>
                ...
                <h1 className='heading  animate__animated animate__fadeInRight' style={{color:"red"}}>"Keylogger"</h1>
                <h1 className="level-indicator  animate__animated animate__fadeInLeft">Cast the spell to proceed:</h1>
              
                <p className=' animate__animated animate__fadeInRight'> 
                A keylogger is a tool used to secretly monitor and record keyboard input, potentially capturing sensitive information and compromising user privacy and security.
                </p>
